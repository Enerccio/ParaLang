...
public class BidiMultiHashMap<K, V> extends AbstractBidiMultiMap<K, V> implements BidiMultiMap<K, V> {
    ...
    protected class EntrySet extends AbstractSet<Entry<K, V>> {
        ...
        public boolean add(Entry<K, V> e) {
            K key = (e.getKey() == null ? (K) NULL : e.getKey());
            V value = (e.getValue() == null ? (V) NULL : e.getValue());

            // Check if the entry already exists.
            int hashCode = hash(key, value);
            int index = hashIndex(hashCode, entries.length);
            HashEntry<K, V> entry = entries[index];
            while (entry != null) {
                if (entry.hashCode() == hashCode && isEqualKey(key, entry.key) && isEqualValue(value, entry.value)) {
        ...
        public boolean contains(Object entry) {
            if (entry instanceof Map.Entry) {
                ...
                return getEntry(e.getKey() != null ? e.getKey() : NULL, e.getValue() != null ? e.getValue() : NULL) != null;
            }
            return false;
        }
        ...
        public boolean remove(Object obj) {
            ...
            Entry e = (Entry) obj;
            Object key = e.getKey() == null ? NULL : e.getKey();
            Object value = e.getValue() == null ? NULL : e.getValue();
            int hashCode = hash(key, value);
            HashEntry<K, V> entry = entries[hashIndex(hashCode, entries.length)];
            while (entry != null) {
                if (entry.entryHashCode == hashCode && isEqualKey(key, entry.key) && isEqualValue(value, entry.value)) {
                    removeMapping(entry);
                    return true;
                }
                entry = entry.entryNext;
    ...
    protected class KeySetIteratorForValue extends HashIteratorForSame<K> {
        ...
        protected HashEntry<K, V> getFirstSame(Object value) {
            ...
            HashEntry<K, V> entry = BidiMultiHashMap.this.values[hashIndex(hashCode, BidiMultiHashMap.this.values.length)];
            while (entry != null) {
                if (entry.valueHashCode == hashCode && isEqualValue(value, entry.value)) {
                    return entry;
                }
    ...
    protected class ValueSetIteratorForKey extends HashIteratorForSame<V> {
        ...
        protected HashEntry<K, V> getFirstSame(Object key) {
            ...
            HashEntry<K, V> entry = BidiMultiHashMap.this.keys[hashIndex(hashCode, BidiMultiHashMap.this.keys.length)];
            while (entry != null) {
	...
	public BidiMultiHashMap(int initialCapacity, float loadFactor) {
        ...
        if (initialCapacity < 1) {
            throw new IllegalArgumentException("Initial capacity must be greater than 0"); //$NON-NLS-1$
        }
        if (loadFactor <= 0.0f || Float.isNaN(loadFactor)) {
            throw new IllegalArgumentException("Load factor must be greater than 0"); //$NON-NLS-1$
        }
        this.loadFactor = loadFactor;
        this.keyThreshold = this.valueThreshold = this.entryThreshold = calculateThreshold(initialCapacity, loadFactor);
    ...
    protected void addMapping(K key, V value) {
        ...
        while (sameKeyEntry != null) {
            if (sameKeyEntry.keyHashCode == keyHashCode && isEqualKey(sameKeyEntry.key, key)) {
                break;
            }
        ...
        while (sameValueEntry != null) {
            if (sameValueEntry.valueHashCode == valueHashCode && isEqualValue(sameValueEntry.value, value)) {
                break;
            }
         ...
         * Create the entry
         */
        HashEntry<K, V> keyPrevious = sameKeyEntry != null ? sameKeyEntry.keyPrevious : null;
        HashEntry<K, V> keyNext = sameKeyEntry != null ? sameKeyEntry.keyNext : this.keys[keyHashIndex];
        HashEntry<K, V> valuePrevious = sameValueEntry != null ? sameValueEntry.valuePrevious : null;
        HashEntry<K, V> valueNext = sameValueEntry != null ? sameValueEntry.valueNext : this.values[valueHashIndex];
        HashEntry<K, V> entry = createEntry(
                keyPrevious,
                keyNext,
                sameKeyEntry,
                valuePrevious,
    ...
    public boolean containsEntry(Object key, Object value) {
        return getEntry(key != null ? key : NULL, value != null ? value : NULL) != null;
    }
    ...
    public boolean containsValue(Object value) {
        ...
        int hashCode = hash(value);
        HashEntry<K, V> entry = this.values[hashIndex(hashCode, this.values.length)];
        while (entry != null) {
            if (entry.valueHashCode == hashCode && isEqualValue(value, entry.value)) {
                return true;
            }
    ...
    protected Iterator<V> createValueSetIteratorForKey(final K key) {
        return new ValueSetIteratorForKey(key);
    }
    ...
    protected void ensureEntryCapacity(int newCapacity) {
        ...
        if (this.size == 0) {
            this.entryThreshold = calculateThreshold(newCapacity, this.loadFactor);
    ...
    protected void ensureValueCapacity(int newCapacity) {
        ...
        if (this.size == 0) {
            this.valueThreshold = calculateThreshold(newCapacity, this.loadFactor);
    ...
    protected HashEntry<K, V> getEntry(Object key, Object value) {
        int hashCode = hash(key, value);
        HashEntry<K, V> entry = this.entries[hashIndex(hashCode, this.entries.length)];
        while (entry != null) {
            if (entry.hashCode() == hashCode && isEqualKey(key, entry.key) && isEqualValue(value, entry.value)) {
                return entry;
            }
    ...
    protected void removeEntry(HashEntry<K, V> entry) {
        ...
        if (entry.keyPrevious != null) {
            if (entry.keyPrevious.keySame == entry) {
                ...
                entry.keyPrevious.keyNext = entry.keySame != null ? entry.keySame : entry.keyNext;
            }
        } else {
            this.keys[hashIndex(entry.keyHashCode, this.keys.length)] = entry.keySame != null ? entry.keySame : entry.keyNext;
        }
        if (entry.keyNext != null) {
            entry.keyNext.keyPrevious = entry.keySame != null ? entry.keySame : entry.keyPrevious;
        }
        if (entry.keySame != null) {
        ...
        if (entry.valuePrevious != null) {
            if (entry.valuePrevious.valueSame == entry) {
                ...
                entry.valuePrevious.valueNext = entry.valueSame != null ? entry.valueSame : entry.valueNext;
            }
        } else {
            this.values[hashIndex(entry.valueHashCode, this.values.length)] = entry.valueSame != null ? entry.valueSame : entry.valueNext;
        }
        if (entry.valueNext != null) {
            entry.valueNext.valuePrevious = entry.valueSame != null ? entry.valueSame : entry.valuePrevious;
        }
        if (entry.valueSame != null) {
    ...
    protected void removeMapping(HashEntry<K, V> entry) {
        this.modCount++;
        boolean sameKeyExits = entry.keySame != null || (entry.keyPrevious != null && entry.keyPrevious.keySame == entry);
        boolean sameValueExits = entry.valueSame != null || (entry.valuePrevious != null && entry.valuePrevious.valueSame == entry);
        removeEntry(entry);
        this.size--;
        if (!sameKeyExits) {
            this.keySize--;
